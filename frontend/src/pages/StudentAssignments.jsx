    ...
    return (
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        className={`relative glass-card p-6 border-2 ${
          isRejected ? 'border-red-400' : isSubmitted ? 'border-green-400' : isPastDue ? 'border-gray-400 opacity-60' : 'border-primary'
        }`}>
        {/* Past due overlay */}
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <h3 className="text-lg font-display font-bold glass-text mb-1">{assignment.title}</h3>
          ...
          <div className="text-right">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
              ...
              isPastDue ? 'bg-gray-500 text-white' : 'bg-primary text-white'
            }`}>
              {isRejected ? 'Rejected' : isSubmitted ? 'Submitted' : isPastDue ? 'Overdue' : 'Pending'}
            </span>
            <p className={`text-sm mt-2 ${isPastDue && !isSubmitted ? 'text-red-500 font-bold' : 'glass-text-muted'}`}>
        ...
        {assignment.description && (
          <p className="text-sm glass-text-muted mb-4">{assignment.description}</p>
        )}
        ...
        <div className="bg-primary rounded-2xl p-6 mb-8 text-white shadow-glass-lg">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/80">{mode === 'subjects' ? 'Select a subject to view assignments' : `${selectedSubject?.subject_code} • Semester ${user?.semester}`}</p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {subjects.length === 0 ? (
              <div className="col-span-full text-center py-12 glass-panel">
                ...
                <p className="glass-text-muted text-lg">No subjects found</p>
              </div>
            ) : (
                ...
                <motion.div key={subject.id} whileHover={{ scale: 1.02 }} onClick={() => handleSubjectClick(subject)}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center">
                      <i className="fas fa-book text-xl text-primary"></i>
                    </div>
                  ...
                  <h3 className="text-lg font-display font-bold glass-text mb-1">{subject.subject_name}</h3>
                  <p className="text-sm glass-text-muted mb-2">{subject.subject_code}</p>
                  <p className="text-sm glass-text-muted">{subject.total_assignments} assignment(s)</p>
                </motion.div>
              ))
            )}
          </div>
            ...
            {assignments.pending.length === 0 && assignments.rejected.length === 0 && 
             assignments.submitted.length === 0 && assignments.overdue.length === 0 && (
              <div className="text-center py-12 glass-panel">
                <i className="fas fa-inbox text-6xl glass-text-muted mb-4"></i>
                <p className="glass-text-muted text-lg">No assignments for this subject</p>
              </div>
            )}
          </div>
        )}
