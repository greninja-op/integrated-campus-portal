    ...
    >
      ...
      <header className="flex flex-wrap gap-3 justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-display font-bold glass-text">Fee Management</h1>
        </div>
        <div className="flex items-center gap-4">
          <ThemeToggle />
      ...
      <div className="glass-panel p-6 mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-display font-bold glass-text">Fee Management System</h2>
              <p className="glass-text-muted">Track pending payments and send fee notices</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-5xl font-display font-bold glass-text">{pendingStudents.length}</p>
            <p className="glass-text-muted">Total Pending Payments</p>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'pending' ? (
        ...
        >
          <div className="flex flex-wrap gap-3 justify-between items-center mb-6">
            <h2 className="text-2xl font-display font-bold glass-text">Students with Pending Payments</h2>
          ...
          <div className="mb-4 p-3 bg-primary/5 rounded-lg">
            <p className="glass-text-muted">
              ...
              Showing <span className="font-bold text-primary">{filteredPendingStudents.length}</span> student(s)
          ...
          {filteredPendingStudents.length === 0 ? (
            <div className="text-center py-12">
              <p className="glass-text-muted text-lg">No pending payments found with current filters!</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-slate-300 dark:border-slate-600">
                    <th className="px-4 py-3 text-left glass-text-muted font-semibold">Roll No</th>
                    ...
                    <th className="px-4 py-3 text-left glass-text-muted font-semibold">Year/Sem</th>
                    <th className="px-4 py-3 text-left glass-text-muted font-semibold">Fee Type</th>
                    <th className="px-4 py-3 text-left glass-text-muted font-semibold">Amount</th>
                    <th className="px-4 py-3 text-left glass-text-muted font-semibold">Due Date</th>
                    <th className="px-4 py-3 text-left glass-text-muted font-semibold">Fine</th>
                    <th className="px-4 py-3 text-left glass-text-muted font-semibold">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredPendingStudents.map((student) => {
                    ...
                    return (
                      <tr key={student.id} className={`border-b border-slate-200 dark:border-slate-700 hover:bg-primary/5 transition-all ${isOverdue ? 'bg-red-100/50 dark:bg-red-900/20' : ''}`}>
                        <td className="px-4 py-3 glass-text font-mono text-sm">{student.rollNo}</td>
                        <td className="px-4 py-3 glass-text font-semibold">{student.name}</td>
                        <td className="px-4 py-3 glass-text">{student.department}</td>
                        <td className="px-4 py-3 glass-text">Y{student.year}/S{student.semester}</td>
                        <td className="px-4 py-3 glass-text">{student.feeType}</td>
                        <td className="px-4 py-3 glass-text font-bold">₹{student.amount.toLocaleString()}</td>
                        <td className="px-4 py-3 glass-text">
                          {new Date(student.dueDate).toLocaleDateString()}
                          {isOverdue && <span className="ml-2 text-danger font-bold">⚠️ OVERDUE</span>}
                        </td>
                        <td className="px-4 py-3 glass-text text-sm">
                          ₹{student.fineAmount} / ₹{student.superFineAmount}
                        </td>
                        <td className="px-4 py-3">
                          <button
                            onClick={() => sendReminderToPending(student)}
                            className="btn-glass px-4 py-2 whitespace-nowrap"
                          >
                            <i className="fas fa-bell mr-2"></i>
                            Send Alert
                          </button>
        ...
        <motion.div 
          ...
          className="glass-panel p-6"
        >
          <h2 className="text-2xl font-display font-bold glass-text mb-6">Create and Send Fee Notice</h2>
          ...
          <div className="space-y-6">
            ...
            >
              <h3 className="text-lg font-display font-bold glass-text mb-4">
            ...
            <motion.div 
              ...
              className="bg-teal-500/5 rounded-xl p-6"
            >
              <h3 className="text-lg font-display font-bold glass-text mb-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <CustomSelect
                  ...
                  onChange={handleInputChange}
                  options={feeTypes}
                  label={<>Fee Type <span className="text-danger">*</span></>}
                  placeholder="Select Fee Type"
                />
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <label className="block glass-text-muted font-semibold mb-2">
                      Super Fine Amount (₹) <span className="text-danger">*</span>
                    </label>
                    <input
                      type="number"
            ...
            <motion.div 
              ...
              className="bg-amber-500/5 rounded-xl p-6"
            >
              <h3 className="text-lg font-display font-bold glass-text mb-4">
